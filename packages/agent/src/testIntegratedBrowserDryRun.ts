/**
 * testIntegratedBrowserDryRun.ts — 到派一体扫描浏览器 DRY-RUN 测试
 *
...
async function main() {
  console.log('═══════════════════════════════════════════');
  console.log('  DaoPai V3 到派一体扫描浏览器 DRY-RUN 测试');
  console.log('  Phase 5-F: 页面级 DRY-RUN，不提交真实到派一体');
  console.log('═══════════════════════════════════════════\n');
  ...
  console.log('[6/7] 执行到派一体浏览器 DRY-RUN...\n');
  const dryRunResult = await runIntegratedBrowserDryRun(page, {
    ...
    options: {
      prevStation: '天津分拨中心',
      // 派件员信息来源：settings.json 中网点的 username 字段（旧代码 IntegratedScan.ts
      // 通过 SettingsManager.resolveWorkerCredential 解析，employeeId 即 username）
      courierName: credential.employeeName,
      courierEmployeeId: credential.loginAccount,
    },
  });
  ...
  console.log(`  上一站选中：${dryRunResult.prevStationSelected ? '是' : '否'}`);
  console.log(`  到派一体勾选：${dryRunResult.integratedCheckboxChecked ? '是' : '否'}`);
  ...
  if (dryRunResult.detectBefore) {
    ...
    console.log(`  是否到派一体页面：${dryRunResult.detectBefore.isIntegratedPage ? '是' : '否'}`);
    console.log(`  上一站输入框：${dryRunResult.detectBefore.hasPrevStationInput ? '已检测到' : '未检测到'}`);
    console.log(`  到派一体复选框：${dryRunResult.detectBefore.hasIntegratedCheckbox ? '已检测到' : '未检测到'}`);
  ...
  console.log('═══════════════════════════════════════════');
  console.log('  到派一体扫描浏览器 DRY-RUN 测试完成');
  console.log('═══════════════════════════════════════════');
  console.log(`  网点：${credential.siteName}`);
  console.log(`  员工：${credential.employeeName}`);
  console.log(`  账号：${maskAccount(credential.loginAccount)}`);
  console.log(`  Dashboard P0：READY`);
  console.log(`  是否进入到派一体页面：${dryRunResult.detectBefore?.isIntegratedPage ? '是' : '否'}`);
  console.log(`  是否识别上一站输入框：${dryRunResult.detectBefore?.hasPrevStationInput ? '是' : '否'}`);
  console.log(`  是否识别到派一体复选框：${dryRunResult.detectBefore?.hasIntegratedCheckbox ? '是' : '否'}`);
  ...
  console.log(`  是否选上一站：${dryRunResult.prevStationSelected ? '是' : '否'}`);
  console.log(`  是否勾选到派一体：${dryRunResult.integratedCheckboxChecked ? '是' : '否'}`);
