...
export async function executeSignDryRun(
  ...
  try {
    ...
    logger.info(`开始签收录入浏览器 DRY-RUN，网点：${siteName}，运单数：${waybills.length}`);
    logger.info('参数校验完成');
    if (staffName) logger.info(`员工：${staffName}`);
    ...
    logger.info('已阻止批量签收（未点击批量签收/确认签收/提交按钮）');
    ...
    logger.success('签收录入浏览器 DRY-RUN 完成，未点击最终提交');
    ...
    if (manager) {
      try {
        ...
        console.error(`[SignExecutor] Chrome 关闭失败：${(closeErr as Error).message}`);
      }
    }
