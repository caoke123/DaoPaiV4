/**
 * ⚠️ LEGACY ONLY — V1/V2 EasyBR compatibility module. DO NOT USE for V4 Agent window lifecycle.
 *
 * M5-2A confirmed: BrowserPool is NEVER reached in V4 playwright mode.
 * - Task execution always routes to PlaywrightWindowAdapter (shouldUsePlaywrightAdapter → always true).
 * - Window startup uses Agent WS commands, not BrowserPool.openBrowerWithRetry().
 * - Window status uses SSE + playwright-windows endpoint, not BrowserPool.
 * - The startup init() call in index.ts fails silently (EasyBR not running).
 *
 * This file is frozen for backwards-compat reference. Do not add new callers.
 */
