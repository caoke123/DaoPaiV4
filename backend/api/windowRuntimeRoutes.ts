...
import { getTenantId } from './middleware/requestContext';
...
// M5-2D: expanded to include all Agent lifecycle states so intermediate states
// survive the API layer intact and reach the frontend for correct badge rendering.
...
// ═══════════════════════════════════════════════════════════
// GET /api/version — 返回后端版本指纹 + 已连接 Agent 的版本信息
// ═══════════════════════════════════════════════════════════
// Agent heartbeat 上报的 gitCommit/buildId/startedAt/chromePath/chromeKind 由
// agentRoutes 保存到 PG，此处通过 getAgentVersionInfo 从 PG 读取最新一条。
import { getRuntimeGitHash, getRuntimeBuildId, backendStartedAt } from '../index';
import { getLatestAgentVersionInfo } from '../agent/agentRoutes';

windowRuntimeRouter.get('/api/version', async (_req: Request, res: Response) => {
  try {
    let agentVersionInfo: Record<string, unknown> | null = null;
    try {
      agentVersionInfo = getLatestAgentVersionInfo();
    } catch {
      // no agent heartbeat yet
    }
    res.json({
      service: 'backend',
      gitCommit: getRuntimeGitHash(),
      buildId: getRuntimeBuildId(),
      startedAt: backendStartedAt,
      runtimeMode: getRuntimeMode(),
      agent: agentVersionInfo ?? { status: 'no_agent_heartbeat_yet' },
    });
  } catch (e) {
...
windowRuntimeRouter.get('/api/sites/:siteId/playwright-windows', async (req: Request, res: Response) => {
  try {
    ...
    // M5-2B: 窗口状态以 Cloud PG (Agent 上报) 为准，不再使用 PlaywrightRuntime 内存缓存
    const pg = PgDatabase.getInstance();
    const tenantId = getTenantId(req) || 'tenant-default';
    // M5-2B: PG stores raw siteId (e.g. "site-1783343430518"), not normalized siteCode ("tiannanda")
    const pgRows = await pg.getWindowStatusBySite(tenantId, siteId);
    const pgStatusMap = new Map<string, typeof pgRows[number]>();
    for (const row of pgRows) {
      pgStatusMap.set(row.windowId, row);
    }

    // M5-2B: Agent 每 5s 上报一次状态。如果超过 300s (5分钟) 没有更新，认为是 stale。
    // 这个阈值比 Agent 上报间隔长得多，给 Agent 重启/网络波动留有裕度。
    const STALE_MS = 300_000;
    const now = Date.now();

    const windows = loginCapableWindows.map(w => {
      const staffName = w.employeeName || w.windowName;
      // M5-2B: PG windowId = settings windowName (e.g. "天南大-肖飞"), not "staff-{name}"
      const pgWindowId = w.windowName;

      // M5-2B: 主数据源 → Cloud PG (Agent 上报)；fallback → PlaywrightRuntime 缓存
      const pgStatus = pgStatusMap.get(pgWindowId);
      let status: WindowState;
      let currentUrl = '';
      let pageCount = 0;
      let activePageUrl = '';
      let p0Passed = false;
      let p0FailedCheck: string | null = null;
      let p0FailedReason: string | null = null;

      if (pgStatus) {
        // Cloud PG has Agent-reported status — authoritative
        const updatedAt = pgStatus.updatedAt ? new Date(pgStatus.updatedAt).getTime() : 0;
        const stale = (now - updatedAt) > STALE_MS;

        if (stale) {
          status = 'offline';
        } else {
          // M5-2D: Preserve PG original status — don't squash intermediate states to "connecting".
          // The frontend WindowStateProvider rounds are already handled in getNodeBadge().
          switch (pgStatus.status) {
            case 'ready': status = 'ready'; break;
            case 'busy': status = 'busy'; break;
            case 'login_required': status = 'login_required'; break;
            case 'failed': case 'error': status = 'degraded'; break;
            case 'closed': case 'closing': status = 'offline'; break;
            case 'offline': status = 'offline'; break;
            default:
              // All intermediate states (opening, process_started, cdp_connecting,
              // cdp_connected, login_checking, p0_checking, popup_cleaning,
              // ready_checking) — pass through as-is so the frontend can render
              // the correct badge (INIT/connecting/etc.) instead of squashing to "connecting".
              status = pgStatus.status as WindowState;
          }
          currentUrl = pgStatus.currentUrl || '';
          p0Passed = pgStatus.isDashboardReady;
        }
      } else {
        // Fallback: PlaywrightRuntime cache (Cloud-side, may be empty for Agent windows)
        const staffWindowId = `staff-${staffName}`;
        const runtimeFallbackKey = buildRuntimeKey(DEFAULT_TENANT_ID, siteCode, staffWindowId);
        const cached = runtime.getWindowStateJSON(runtimeFallbackKey);
        if (!cached) {
          status = 'offline';
        } else {
          ...
          switch (pwStatus) {
            case 'launching': case 'logging_in': adapterStatus = 'opening'; break;
            case 'ready': adapterStatus = 'ready'; break;
            case 'busy': adapterStatus = 'busy'; break;
            case 'login_required': adapterStatus = 'login_required'; break;
            case 'closed': adapterStatus = 'closed'; break;
            case 'error': default: adapterStatus = 'failed'; break;
          }
      ...
      console.log(
        `[playwright-windows] ${staffName} | pgWindowId=${pgWindowId} | ` +
        `pgStatus=${pgStatus?.status ?? '(none)'} → frontend=${status}`,
      );
      ...
      return {
        ...
        runtimeKey: buildRuntimeKey(DEFAULT_TENANT_ID, siteCode, w.windowName),
        currentUrl,
        pageCount,
        activePageUrl: activePageUrl || currentUrl,
        p0Passed,
        p0FailedCheck,
        p0FailedReason,
      };
...
windowRuntimeRouter.post('/api/sites/:siteId/playwright-windows/launch-all', async (req: Request, res: Response) => {
  try {
    ...
    const siteCode = normalizeSiteNameToCode(site.name);
    // M5-2D: PG instance for upserting window status after launch+login
    const pg = PgDatabase.getInstance();
    ...
    for (const target of launchTargets) {
      try {
        ...
        // M5-2D: Upsert window status to PG so frontend playwright-windows API
        // sees fresh data. Without this, the Agent (which normally reports status)
        // won't have these windows in its registry, causing stale PG data → offline.
        const stateForPg = runtime.getWindowState(r.runtimeKey);
        pg.upsertWindowStatus({
          tenantId: DEFAULT_TENANT_ID,
          siteId,
          workstationId: 'ws-local-default',
          windowId: target.windowName,
          staffName: target.staffName,
          status: finalStatus,
          statusText: loginResult.message || r.message || '',
          currentUrl: stateForPg?.currentUrl,
          isProcessAlive: true,
          isCdpReady: true,
          isDashboardReady: isReady,
          isLoginPage: stateForPg?.isLoginPage ?? false,
        }).catch(err => console.error(`[playwright-launch-all] PG upsert failed for ${target.staffName}:`, (err as Error).message));
      } catch (e) {
        failed++;
