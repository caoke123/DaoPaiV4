...
function notifyAgentTaskAvailable(input: {
  tenantId: string;
  workstationId: string;
  taskId: string;
  taskType: string;
  siteId: string;
}): { agentOnline: boolean; wsPushed: boolean } {
  const agentWs = getAgentWebSocket();
  const agentOnline = agentWs.isAgentOnline(input.tenantId, input.workstationId);
  const wsPushed = agentOnline
    ? agentWs.pushTaskAvailable({
        id: input.taskId,
        type: input.taskType,
  ...
  logTrace('task-notify', 'task_available_push_result', {
    taskId: input.taskId,
...
router.get('/api/status', async (_req: Request, res: Response) => {
  ...
  // M5-2B: 检查 Agent 是否在线（Cloud PG workstation 心跳）→ 决定 runtime 可用性
  // 不再仅依赖 BrowserPool（Agent 窗口不在 BrowserPool 中）
  let agentAvailable = false;
  try {
    const pg = PgDatabase.getInstance();
    const tenantId = getTenantId(_req) || 'tenant-default';
    const HEARTBEAT_WINDOW_MS = 120_000;
    const result = await pg.getPool().query(
      `SELECT last_heartbeat_at FROM workstations
       WHERE tenant_id = $1 AND last_heartbeat_at > NOW() - INTERVAL '${Math.ceil(HEARTBEAT_WINDOW_MS / 1000)} seconds'
       LIMIT 1`,
      [tenantId],
    );
    agentAvailable = (result.rowCount ?? 0) > 0;
  } catch {
    // PG 不可用 → 回退原有逻辑
  }

  const runtime: { runtime: string; runtimeError: string | null } = agentAvailable
    ? { runtime: 'available', runtimeError: null }
    : (() => {
        const s = runtimeStatus.getSummary();
        return { runtime: s.runtime, runtimeError: s.runtimeError };
      })();
  ...
  try {
    ...
    // BrowserPool 不可用 → Agent 状态决定 runtime
    res.json({
      alive: true,
      authRequired,
      runtime: runtime.runtime,
 ...
 *  M5-2B: 若 Agent 超过 300s (5分钟) 未上报，标记 stale=true 并降级为 offline。
 *  注意：与 playwright-windows 端点的 STALE_MS 保持一致。
 */
router.get('/api/cloud/windows/status', async (req: Request, res: Response) => {
  try {
    ...
    // M5-2B: 与 playwright-windows 端点保持一致，给 Agent 重启/网络波动留足裕度
 ...
 *
 * Header 启动/关闭窗口不再直接操作 PlaywrightRuntime，
 * 而是创建一条 window_command，由 Agent 拉取并执行。
 */
router.post('/api/cloud/windows/commands', async (req: Request, res: Response) => {
  try {
    const { siteId, windowId, staffName, type, workstationId } = req.body || {};

    if (!siteId || typeof siteId !== 'string') {
      return res.status(400).json({ error: '缺少 siteId' });
    }
    if (!windowId || typeof windowId !== 'string') {
      return res.status(400).json({ error: '缺少 windowId' });
    }
    if (!staffName || typeof staffName !== 'string') {
      return res.status(400).json({ error: '缺少 staffName' });
    }
    if (!type || !['open_window', 'close_window', 'restart_window', 'refresh_status'].includes(type)) {
      return res.status(400).json({ error: 'type 必须为 open_window / close_window / restart_window / refresh_status' });
    }

    const pg = PgDatabase.getInstance();
    const tenantId = getTenantId(req);
    // workstationId: 当前开发环境使用默认值，后续由租户配置决定
    const wsId = workstationId && typeof workstationId === 'string' ? workstationId : 'ws-local-default';

    const commandId = randomUUID();
    const traceScope = 'window-command';
    logTrace(traceScope, 'create_requested', {
      commandId,
      tenantId,
      siteId,
      workstationId: wsId,
    ...
    logTrace(traceScope, 'create_inserted', {
      commandId,
      tenantId,
      siteId,
      workstationId: wsId,
    ...
    // S2: 尝试通过 WebSocket 推送给 Agent
    const agentWs = getAgentWebSocket();
    const agentOnline = agentWs.isAgentOnline(tenantId, wsId);
    let wsPushed = false;
    if (agentOnline) {
      wsPushed = agentWs.pushCommand({
        id: commandId,
        type,
        tenantId,
        siteId,
    ...
    logTrace(traceScope, 'create_ws_push_result', {
      commandId,
      tenantId,
      siteId,
      workstationId: wsId,
    ...
    res.json({
      commandId,
      status: 'pending',
      agentOnline: agentOnline || wsPushed,
      message: `窗口命令 ${type} 已创建${agentOnline ? '，已推送至本地执行套件' : ''}`,
    });
  } catch (e) {
    console.error('[POST /api/cloud/windows/commands] 失败:', (e as Error).message);
    res.status(500).json({ error: (e as Error).message });
  }
});
 ...
 *
 * 用于一键启动：为当前站点所有员工窗口批量创建 open_window command。
 */
router.post('/api/cloud/windows/commands/batch', async (req: Request, res: Response) => {
  try {
    const { commands } = req.body || {};

    if (!commands || !Array.isArray(commands) || commands.length === 0) {
      return res.status(400).json({ error: '缺少 commands 数组' });
    }

    if (commands.length > 50) {
      return res.status(400).json({ error: '一次最多创建 50 条命令' });
    }

    const pg = PgDatabase.getInstance();
    const tenantId = getTenantId(req);
    const results: Array<{ commandId: string; windowId: string; staffName: string }> = [];
    const pushedWorkstations = new Set<string>();
    let agentOnlineCount = 0;
    let wsPushedCount = 0;
    const agentWs = getAgentWebSocket();

    for (const cmd of commands) {
      if (!cmd.siteId || !cmd.windowId || !cmd.staffName || !cmd.type) continue;

      const commandId = randomUUID();
      const wsId = cmd.workstationId && typeof cmd.workstationId === 'string' ? cmd.workstationId : 'ws-local-default';

      await pg.createWindowCommand({
        id: commandId,
        tenantId,
        siteId: String(cmd.siteId),
        workstationId: wsId,
        windowId: String(cmd.windowId),
        staffName: String(cmd.staffName),
        type: String(cmd.type),
      });
      logTrace('window-command-batch', 'create_inserted', {
        commandId,
        tenantId,
        siteId: String(cmd.siteId),
        workstationId: wsId,
      ...
      if (agentOnline) {
        ...
        wsPushed = agentWs.pushCommand({
          id: commandId,
          type: String(cmd.type) as 'open_window' | 'close_window' | 'restart_window' | 'refresh_status',
          tenantId,
          siteId: String(cmd.siteId),
          workstationId: wsId,
          windowId: String(cmd.windowId),
        ...
        if (wsPushed) wsPushedCount += 1;
      }
      pushedWorkstations.add(wsId);
      logTrace('window-command-batch', 'create_ws_push_result', {
        commandId,
        tenantId,
        siteId: String(cmd.siteId),
        workstationId: wsId,
      ...
      results.push({ commandId, windowId: String(cmd.windowId), staffName: String(cmd.staffName) });
    }

    logTrace('window-command-batch', 'batch_created', {
      tenantId,
      commandCount: results.length,
      workstationCount: pushedWorkstations.size,
      agentOnlineCount,
      wsPushedCount,
    });
    res.json({
    ...
    console.error('[POST /api/cloud/windows/commands/batch] 失败:', (e as Error).message);
    res.status(500).json({ error: (e as Error).message });
  }
});
 ...
 * Header 用于轮询命令执行状态。
 */
router.get('/api/cloud/windows/commands/:commandId', async (req: Request, res: Response) => {
  try {
    const { commandId } = req.params;

    const pg = PgDatabase.getInstance();
    const cmd = await pg.getWindowCommandById(commandId);
    ...
    console.error('[GET /api/cloud/windows/commands/:id] 失败:', (e as Error).message);
    res.status(500).json({ error: (e as Error).message });
  }
});

/**
 * GET /api/cloud/windows/events — SSE 实时事件流 (Deploy-0D-S2)
 *
 * Header/WindowStateProvider 建立 EventSource 订阅。
 * 推送事件类型：
 *   - agent_connected / agent_disconnected
 *   - command_claimed / command_running / command_done / command_failed
 *   - window_status_updated
 */
router.get('/api/cloud/windows/events', (req: Request, res: Response) => {
  ...
  const send = (data: string) => {
    try { res.write(data); } catch { /* ignore closed */ }
  };

  const clientId = addSseClient(siteId, send);

  // 连接时推送初始状态
  send(`event: connected\ndata: {"siteId":"${siteId}","at":"${new Date().toISOString()}"}\n\n`);

  req.on('close', () => {
    clearInterval(heartbeat);
    removeSseClient(siteId, send);
  });
...
router.post('/api/operations/arrive', async (req: Request, res: Response) => {
  ...
  // Phase 2-C: PG 主写 — 预生成 UUID，先写 PG（PRIMARY），失败直接 500
  const taskId = randomUUID();
  logTrace('task-arrival', 'task_create_requested', {
    taskId,
    tenantId: getTenantId(req),
    workstationId: getWorkstationId(req),
    siteId: siteCode,
    totalCount,
    assignmentCount: finalAssignments.length || null,
  });
  ...
  const taskNotify = notifyAgentTaskAvailable({
    tenantId: getTenantId(req),
    workstationId: getWorkstationId(req),
    taskId,
    taskType: 'arrival',
  ...
  logTrace('task-arrival', 'task_create_done', {
    taskId,
    tenantId: getTenantId(req),
    workstationId: getWorkstationId(req),
    siteId: siteCode,
    status: 'pending',
    agentOnline: taskNotify.agentOnline,
    wsPushed: taskNotify.wsPushed,
  });
...
router.post('/api/operations/dispatch', async (req: Request, res: Response) => {
  ...
  // Phase 2-C: PG 主写 — 预生成 UUID，先写 PG（PRIMARY），失败直接 500
  const taskId = randomUUID();
  logTrace('task-dispatch', 'task_create_requested', {
    taskId,
    tenantId: getTenantId(req),
    workstationId: getWorkstationId(req),
    siteId: siteCode,
  ...
  // 3. 立即返回，任务保持 pending 状态
  logTrace('task-dispatch', 'task_create_done', {
    taskId,
    tenantId: getTenantId(req),
    workstationId: getWorkstationId(req),
    siteId: siteCode,
...
router.post('/api/operations/integrated', async (req: Request, res: Response) => {
  ...
  // Phase 2-C: PG 主写 — 预生成 UUID，先写 PG（PRIMARY），失败直接 500
  const taskId = randomUUID();
  logTrace('task-integrated', 'task_create_requested', {
    taskId,
    tenantId: getTenantId(req),
    workstationId: getWorkstationId(req),
    siteId: siteCode,
  ...
  // 3. 立即返回，任务保持 pending 状态
  logTrace('task-integrated', 'task_create_done', {
    taskId,
    tenantId: getTenantId(req),
    workstationId: getWorkstationId(req),
    siteId: siteCode,
...
router.post('/api/operations/sign', async (req: Request, res: Response) => {
  ...
  // Phase 2-C: PG 主写 — 预生成 UUID，先写 PG（PRIMARY），失败直接 500
  const taskId = randomUUID();
  logTrace('task-sign', 'task_create_requested', {
    taskId,
    tenantId: getTenantId(req),
    workstationId: getWorkstationId(req),
    siteId: siteCode,
  ...
  // 3. 立即返回，任务保持 pending 状态
  logTrace('task-sign', 'task_create_done', {
    taskId,
    tenantId: getTenantId(req),
    workstationId: getWorkstationId(req),
    siteId: siteCode,
...
router.post('/api/cloud/agent-test-task', async (req: Request, res: Response) => {
  try {
    ...
    res.json({ ok: true, taskId, message: '测试任务已创建', agentOnline: taskNotify.agentOnline, wsPushed: taskNotify.wsPushed });
  } catch (e) {
...
router.post('/api/cloud/agent-arrival-task', async (req: Request, res: Response) => {
  try {
    ...
    res.json({ ok: true, taskId, message: 'Arrival DRY-RUN 任务已创建', waybillCount: waybills.length, dryRunMode: resolvedDryRun, agentOnline: taskNotify.agentOnline, wsPushed: taskNotify.wsPushed });
  } catch (e) {
...
router.post('/api/cloud/agent-dispatch-task', async (req: Request, res: Response) => {
  try {
    ...
    res.json({ ok: true, taskId, message: 'Dispatch DRY-RUN 任务已创建', waybillCount: waybills.length, dryRunMode: resolvedDryRun, agentOnline: taskNotify.agentOnline, wsPushed: taskNotify.wsPushed });
  } catch (e) {
...
router.post('/api/cloud/agent-integrated-task', async (req: Request, res: Response) => {
  try {
    ...
    res.json({ ok: true, taskId, message: 'Integrated DRY-RUN 任务已创建', waybillCount: waybills.length, dryRunMode: resolvedDryRun, agentOnline: taskNotify.agentOnline, wsPushed: taskNotify.wsPushed });
  } catch (e) {
...
router.post('/api/cloud/agent-sign-task', async (req: Request, res: Response) => {
  try {
    ...
    res.json({ ok: true, taskId, message: 'Sign DRY-RUN 任务已创建', waybillCount: waybills.length, dryRunMode: resolvedDryRun, agentOnline: taskNotify.agentOnline, wsPushed: taskNotify.wsPushed });
  } catch (e) {
