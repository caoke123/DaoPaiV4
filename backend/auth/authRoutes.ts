...
authRouter.post('/api/auth/login', async (req: Request, res: Response) => {
  try {
    ...
    // #region debug-point H4:backend-login-catch
    (() => { fetch('http://127.0.0.1:7777/event', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ sessionId: 'frontend-login-flow', runId: 'post-fix', hypothesisId: 'H4', location: 'backend/auth/authRoutes.ts:/api/auth/login', msg: '[DEBUG] backend login threw error', data: { error: (err as Error).message }, ts: Date.now() }) }).catch(() => {}); })();
    // #endregion
